...
"""
    ...
    self.assert_( r.length() < 1000 )
    self.assert_( r.findNearestEx( (14.498925, 50.084739999999996) )[2] < 10 )

  def testLine( self ):
    line = Line( (1,2), (3,4) )
    self.assertAlmostEqual( line.signedDistance( (2,3) ), 0.0, 10 )
    self.assert_( line.signedDistance( (0, 10) ) > 0 )

  def testLineFinished( self ):
    line = Line( (0,0), (10,0) )
    self.assert_( not line.finished( ( 5.0, 0.0 ) ) )
    self.assert_( line.finished( ( 11.0, 0.0 ) ) )
    self.assert_( line.finished( ( 11.0, 1.0 ) ) )
